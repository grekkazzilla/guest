...
<script type='text/javascript'>
  ...
  function init(){
    ...
    // DIV WAIT
    var div=getG('divWait',gWrap,0,0,1,false,OBJ.w/2,OBJ.h/2);
    getPath(null,div,200-picFence()[1]*0.15/2,10,0.15,'#bdb76d','none',0,picFence()[2]);
    //
    var gWhiteKnight=getG(null,div,0,0,0,true,504/2,504/2);
    ...
    gWhiteKnight.setAttribute('transform','translate(195,80) scale(-0.25,0.25)');
    //
    var gBlackKnight=getG(null,div,210,80,0.25,true,504/2,504/2);
    ...
    var box=getG('boxWait',div,X,Y+(H+S),1,true,W/2,H/2);
    box.objWatch=null;
    getRect(null,box,0,0,W,H,0,'#808080','none',0).setAttribute('filter','url(#blr2)');
    getRect(null,box,0,0,W,H,0,'#fff','none',0);
    getButton(null,box,5,5,105,105,false,'CXAX',picNone(),0,function(){},null);
    getButton(null,box,W-65,5,60,60,true,'CXDX',picChatA(),0.13,function(){

    },null);
    getText(null,box,130,25,18,'Arial','url(#grdIcon)','none',0,'','start');
    //for(var j=0;j<5;j++){
    //  getPath('pthWaitStar'+i+j,box,130+35*j,33,0,'url(#grdGold)','none',0,picStar()[2]);
    //}
    getText(null,box,130,95,18,'Arial','url(#grdIcon)','none',0,'','start').style.fontWeight='bold';
    getRect(null,box,115,5,205,60,5,'transparent','#bdb76d',1);
    getRect(null,box,115,70,270,40,5,'transparent','#bdb76d',1);
    var w=60, h=40, icn=getG(null,box,W-65,70,1,false,w/2,h/2);
    draw_white_pawn('gWhiteWait',icn,(w-50)/2,(h-50)/2,0.985,true); // x-9
    draw_black_pawn('gBlackWait',icn,(w-50)/2,(h-50)/2,1,true); // x+9
    ...
    var wIn=130, hIn=50, gIn=getG(null,g,150,0,1,false,wIn/2,hIn/2);
    ...
    //getButton(null,div,135,530,130,60,true,'DXEX',picNo(),0.16,function(){},null);
    getButton(null,div,205-35,530,60,60,true,'AXEX',picNo(),0.16,function(){},null);
    //
    OBJ_host.get();
    OBJ_host.putName();
