...
function texted(conn,data){
  var asw=data.split('~');
  if(asw[0]=='watch'){
    if(OBJ_var.strMode=='wait'){
      conn.send('arena~'+OBJ_var.intVar);
      linked(conn);
    }
  }
  else if(asw[0]=='arena'){
...
function addFind(objClub){
  for(var i=0;i<3;i++){
    var gWatch=o('gWatch'+i);
    if(gWatch.objClub===null){
      var txt=gWatch.getElementsByTagName('text')[0];
      txt.firstChild.nodeValue=strName;
      gWatch.objClub=objClub;
    }
  }
}
